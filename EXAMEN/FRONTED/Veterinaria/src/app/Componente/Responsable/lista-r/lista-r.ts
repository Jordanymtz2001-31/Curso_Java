...
@Component({
  selector: 'app-lista-r',
  imports: [FormsModule],
  templateUrl: './lista-r.html',
  styleUrl: './lista-r.css',
})
export class ListaR implements OnInit{
  ...
  responsables!: Responsable[]; //Creamos una listar para almacenar
  responsable = new Responsable(); //Inicializamos un objeto para usarlo
  ...
  ngOnInit(): void{
    this.servicio.listarResponsable().subscribe({
      next: (data) => { //data es la respuesta del servidor
        this.responsables = data; //Asignamos los datos a la variable responsables
      },
      error: (err) => { console.error('Error al cargar los responsables', err);
      }
    })
  }
  ...
  btnEliminar(id: number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.servicio.eliminarResponsable(id).subscribe({
          next: (response) => {
            const mensaje = response.body?.message || response.body || 'El Responsable ha sido eliminada correctamente';
            Swal.fire(
              'Eliminada',
              mensaje,
              'success'
            );
            this.ngOnInit(); //Recargamos la lista de responsables despues de eliminar
          },
          error: (err) => {
            const errorMsg = err.error?.error || err.error || 'Hubo un problema al eliminar el responsable';
            Swal.fire(
              'Error',
              errorMsg,
              'error'
        ...
        Swal.fire({
          title: 'Cancelado',
          icon : 'info',
          text: 'El Responsable no ha sido eliminado',
  ...
  //Metodo para Listar Mascotas dependiendo del Responsable
  btnListMascota(id: number){
    this.router.navigate(['detalle/responsables', id]);
  }
}
